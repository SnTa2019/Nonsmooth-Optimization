c<><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><>
c    Sharp augmented Lagrangian based method for constrained DC optimization (SHL-DC)
c<><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><>
c 
c<><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><>
!*************************************************************************
! Following functions are used to define objective and constraints 
! functions and their subgradients
!*************************************************************************
c<><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><><>

c    func1(x,f)                                                         ! First DC component of objective f1
c    func2(x,f)                                                         ! Second DC component of objective f2
c    gradient1(x,grad)                                                  ! Gradient of DC componenet of f1 
c    gradient2(x,grad)                                                  ! Gradient of DC componenet of f2 
c    const(x,g1,g2)                                                     ! g(const) = g1(const) -g2(const) 
c    constgrad(x,grad1,grad2)                                           ! Gradient of g1 and g2
c    const12(x,h1,h2)                                                   ! Functions g1+ and g2+
c    gradientC1(x,grad)                                                 ! Gradient of g1+
c    gradientC2(x,grad)                                                 ! Gradient of g2+
c    funcp1(x,f)                                                        ! First DC component of the penalty function  
c    funcp2(x,f)                                                        ! Second DC component of the penalty function  
c    gradp1(x,grad)                                                     ! Gradient of the first DC component of the penalty function  
c    gradp2(x,grad)                                                     ! Gradient of the second DC component of the penalty function

!*************************************************************************
! Following functions are used in optimization process 
!*************************************************************************
c    sharp(n)
c    optimum(x,nbundle)
c    wolfe(ndg,prod)
c    equations(n,z1)
c    subgrad(nsub,x,tau,g,v,grad2)
c    armijo(x,g,f1,f5,f4,tau,sigma,r)
c    esubdif(tau,nsub1,nsub2,x,w1,w2)
c    subgradsub(f1,tau,x,g,v)
c    armijosub(x,g,f1,f5,f4,tau,sigma,r)
c    underestim(hbar)
c    optimest(x,nbundle)
c    fest(y,f)
c    subgradest(f1,tau,x,g,v)
c    armijoest(x,g,f1,f5,f4,tau,sigma,r)
c    linesearch(fvalue,g,sk)
c    augsub(x,nbundle,f)                                                
c    TMPBNGC() 
...
c=====================================================================
      ...
      double precision spoint(20,1000)
      COMMON /cnselect/nselect ,/cstart/spoint,/cnstart/npoint
     1 ,/cnconst/nconst 
      open(42,file='Results.txt')
      open(78,file='startpoints10.txt',status='old',form='formatted')
c      open(78,file='startpointsexp.txt',status='old',form='formatted')  ! use this random starting point for DC2-4 (problem 9)
c======================================================
c Input data   
c----------------------------------------------------------------------
      write(42,141) 
141   format('Prob  ','   n','           In. obj.','       In. const.'
     1 ,'      Optval-SL','       Const-SL'
     1 ,'      Optimal val.','   Final const.','   #f1.eval.'
     2 ,'  #f2.eval.','   #f1.grad','   #f2.grad','   #con.eval.'
     3 ,'   #g1.grad','    #g2.grad','     CPU')
      write(42,*)   
c----------------------------------------------------------------------
      PRINT *,' '
      ...
      if(nstarting.eq.2) then
       ...
       READ *,npoints   !Many starting points      
      end if
c=====================================================================
      if(nstarting.eq.2) then      
       nstart=npoints
  ...
  901  WRITE(42,*) 'Input complete. Number of records: ',npoint
       WRITE(42,*)
      end if 
c======================================================================
      j=21                   ! 21 problems
      do i=1,j
       nselect=i
c       if(nselect.eq.9) go to 111
!***********************************************************************
! Define number of variables and number of constraints for test problems 
!***********************************************************************
! GROUP 1 DC1-1 (nselect=i=1) to DC1-5 (nselect=i=5)
!**********************************************************************          
       if(i.eq.1) then                                    !   (DC1-1)
           n=2
           nconst=2
           call sharp(n)
       end if
       
       if(i.eq.2) then                                    !   (DC1-2)
           n=2
           nconst=2
           call sharp(n)
       end if 
       
       if(i.eq.3) then                                    !   (DC1-3)
           n=4
           nconst=3
           call sharp(n)
       end if 
        
       if(i.eq.4) then                                    !   (DC1-4)
           n=4
           nconst=3
           call sharp(n)
       end if 
       
       if(i.eq.5) then                                    !   (DC1-5)
           n=3
           nconst=4
           call sharp(n)
       end if  
!**********************************************************************
! GROUP 2 DC2-1 (nselect=i=6) to DC2-4 (nselect=i=9)
!**********************************************************************          
       if(i.eq.6) then                                    !   (DC2-1)     
           n=3
           nconst=3
           call sharp(n)
       end if
       
       if(i.eq.7) then                                    !   (DC2-2) 
           n=3
           nconst=2
           call sharp(n)
       end if 
       
       if(i.eq.8) then                                    !   (DC2-3) 
           n=7
           nconst=4
           call sharp(n)
       end if  
       
       if(i.eq.9) then                                    !   (DC2-4) 
           nconst=2
           do k=1,7
            if(k.eq.1) n=2
            if(k.eq.2) n=5
...
!**********************************************************************
! GROUP 3 DC3-1 (nselect=i=10) to DC3-12 (nselect=i=21)
!**********************************************************************       
       if(i.eq.10) then                                    !   (DC3-1) 
           n=3
           nconst=3
           call sharp(n)
       end if
       
       if(i.eq.11) then                                    !   (DC3-2) 
           nconst=3
           do k=1,7
            if(k.eq.1) n=2
            if(k.eq.2) n=5
       ...
       if(i.eq.12) then                                    !   (DC3-3) 
           n=3
           nconst=5
           call sharp(n)
       end if  
       
       if(i.eq.13) then                                    !   (DC3-4) 
           n=2
           nconst=3
           call sharp(n)
       end if 
       
       if(i.eq.14) then                                    !   (DC3-5) 
           n=10
           nconst=4
           call sharp(n)
       end if 
       
       if(i.eq.15) then                                    !   (DC3-6)
           nconst=2
           do k=1,7
            if(k.eq.1) n=2
            if(k.eq.2) n=5
       ...
       if(i.eq.16) then                                    !   (DC3-7) 
           n=2
           nconst=2
           call sharp(n)
       end if  
       
       if(i.eq.17) then                                    !   (DC3-8) 
           n=4
           nconst=1
           call sharp(n)
       end if
        
       if(i.eq.18) then                                    !   (DC3-9) 
           n=2
           nconst=2
           call sharp(n)
       end if 
       
       if(i.eq.19) then                                    !   (DC3-10) 
           n=2
           nconst=2
           call sharp(n)
       end if 
         
       if(i.eq.20) then                                    !   (DC3-11) 
           n=2
           nconst=2
           call sharp(n)
       end if
        
       if(i.eq.21) then                                    !   (DC3-12) 
           n=4
           nconst=1
           call sharp(n)
       end if 
...
c--------------------------------------------------------------------------
     ...
     1 ,spoint(20,1000)
      COMMON /csize/m,/cbox/xl,xu,/cpen/pen,/cpenbox/penbox
     1 ,/cnf1/nf1,/cnf2/nf2,/cngrad1/ngrad1,/cngrad2/ngrad2,/cuc/u,c
     2 ,/cnselect/nselect,/cxbar/xbar,/cind/ind,/cnstart/nstart
     3 ,/cind1/ind1,/cstart/spoint,/cncon/ncon,/cncong1/ncong1
     4 ,/cncong2/ncong2
c=====================================================================
       do i=1,n
        xl(i) = -1.0d+02
       ...
       if(nselect.eq.9) then                                            !   (DC2-4)    ! box is considered smaller due to EXP function 
        do i=1,n
          xl(i) = -5.0d+00
          xu(i) = 5.0d+00
        end do       
...
c=====================================================================
      do istart=1,nstart
       ...
       if(nstart.eq.1) then
!*********************************************************************** 
! Define starting points for test problems        
!*********************************************************************** 
       SELECT CASE(nselect) 
!**********************************************************************
! GROUP 1 DC1-1 (nselect=i=1) to DC1-5 (nselect=i=5)
...
cc**********************************************************************
c! GROUP 2 DC2-1 (nselect=i=6) to DC2-4 (nselect=i=9)
c!**********************************************************************          
       CASE(6)                                    !   (DC2-1)  
        x(1)=1.0d+00          
        x(2)=1.0d+00 
...
c!**********************************************************************
c! GROUP 3 DC3-1 (nselect=i=10) to DC3-12 (nselect=i=21)
c!********************************************************************** 
      CASE(10)                                    !   (DC3-1) 
        x(1)=2.0d+00
        x(2)=2.0d+00
...
!***********************************************************************                                                          
       ...
       pen=1.0d+02
       if(nselect.eq.3) pen=3.0d+02
       if(nselect.eq.4) pen=5.0d+02
       if(nselect.eq.5) pen=1.0d+03
       if(nselect.eq.14) pen=1.0d+03
       if(nselect.eq.9) pen=5.0d+02
       if(nselect.eq.15) pen=5.0d+02
       if(nselect.eq.20) pen=2.0d+02  
       ind=2
       ind1=1
       call augsub(x,fvalue)                                    
       call const12(x,g1,g2)
       g=g1-g2
       call cpu_time(time4)
       time5=time4-time1

       WRITE(42,41) nprob1,n,finit,ginit,fsl,gsl,fvalue,g,nf1,nf2,ngrad1
     1  ,ngrad2,ncon,ncong1,ncong2,time5
      END DO
  41  FORMAT(i4,i6,f20.4,f15.5,f15.6,f15.5,f15.6,f15.5,7i12,f12.4)
      RETURN
      END
...
c--------------------------------------------------------------------------
      subroutine linesearch(x,g1)
      implicit double precision (a-h,o-z)
      PARAMETER(maxvar=2000, maxdg=1000, maxcon=100)
      double precision x(maxvar),x1(maxvar),x2(maxvar),w1(maxdg,maxvar)
     1 , w2(maxdg,maxvar), x3(maxvar)
      COMMON /csize/m,/cuc/u,c,/cw12/w1,w2,/cnsub/nsub
       beta=1.0d-02                                             !
       teta=1.5d+00                                             !sigma
       eps=1.0d-03                                              !delta
       k=0
  ...
  1    step=step*teta
       ...
       u=dmax1(0.0d+00,u-step*g)
       c=dmin1(1.0d+02,c+step*g)
       do i=1,m
        x3(i)=x1(i)
       end do
       ...
       hbar2=hbar1-hbar-beta*step*g
       if(hbar2.le.0.0d+00) then
        do i=1,m
         x(i)=x1(i)
        end do
        return
      ...
      implicit double precision (a-h,o-z)
      PARAMETER(maxvar=2000, maxdg=1000, maxcon=100)
      double precision x(maxvar), x1(maxvar), x2(maxvar)  
      COMMON /csize/m,/cnsub/nsub,/ccur/icur,jcur
       do i=1,m
        x2(i)=x(i)
       end do
       f1=1.0d+30
...
c==================================================================
      ...
      double precision x(maxvar), v1(maxvar), d(maxvar), x3(maxvar)
     1 , w1(maxdg,maxvar), w2(maxdg,maxvar), v2(maxvar)
      common /csize/m,/cw12/w1,w2,/cnsub/nsub,/cxbar/x
c====================================================================
      nlimit=min(3,m)
      m1=2*m
      nsub=0
...
c=====================================================================
      ...
      implicit double precision (a-h,o-z)
      PARAMETER(maxvar=2000,maxdg=1000)
      double precision x(maxvar), w1(maxdg,maxvar), w2(maxdg,maxvar)
     1 ,xbar(maxvar),xl(maxvar),xu(maxvar)
      COMMON /csize/m,/cw12/w1,w2,/cxbar/xbar,/cuc/u,c,/cbox/xl,xu
     1 ,/ccur/icur,jcur,/cpenbox/penbox  
       call func1(x,f1)
       ...
       d2=0.0d+00
       do k=1,m
          d1=d1+(w1(icur,k)+(c-u)*w2(jcur,k))*(x(k)-xbar(k))
...
c=====================================================================
     ...
     1 ,gradh1(maxvar), w1(maxdg,maxvar),w2(maxdg,maxvar),xl(maxvar)
      common /csize/m,/cuc/u,c,/ccur/icur,jcur,/cw12/w1,w2,/cbox/xl,xu
     1 ,/cpenbox/penbox
      call gradient1(x,gradf1)
      call gradientC1(x,gradh1)
      do k=1,m
       v(k)=gradf1(k)+(c-u)*gradh1(k)-w1(icur,k)-(c-u)*w2(jcur,k)
      end do
      do k=1,m
...
c  subgrad, armijo.  
c Proximal Boundle method (TMPBNGC) is applied as a local search
c================================================================
      subroutine augsub(x,f)                    
      implicit double precision (a-h,o-z)
      PARAMETER(maxvar=2000)
...
c=====================================================================
c Subroutine subgrad calculates subgradients 
c=====================================================================
...
c -------------------------------------------------------------------------- 
     ...
     1 ,grad2(maxvar)
      COMMON /csize/m,/cpen/pen 
      call gradient2(x,grad0)
      call gradientC2(x,grad2)
      do i=1,m
       grad(i)=grad0(i)+pen*grad2(i)
      end do
      return
      end
c=================================================================
...
c--------------------------------------------------------------------------
! DC components of objective function f: f1 & f2 (box for X is added to f1)
c--------------------------------------------------------------------------
      subroutine func1(x,f)
      implicit double precision (a-h,o-z)
      PARAMETER(maxvar=2000)
      double precision x(maxvar),XL(MAXVAR),XU(MAXVAR)
...
!**********************************************************************          
        CASE(1)                                                         !   (DC1-1)      
           z1=x(1)**2-x(2)
           z2=(x(1)-1.0d+00)**2
           f=z2+2.0d+02*dmax1(0.0d+00,z1)
                  
        CASE(2)                                                         !   (DC1-2)
           f=ABS(x(1)-1.0d+00)+200.0d+00*dmax1(0.0d+00,ABS(x(1))-x(2)) 
     
        CASE(3)                                                         !   (DC1-3)      
           f=(x(1)-1.0d+00)**2+(x(3)-1.0d+00)**2
     1      +4.95d+00*(x(2)**2+x(4)**2-2.0d+00)
     2      +10.1d+00*((x(2)-1.0d+00)**2+(x(4)-1.0d+00)**2)
     3      +2.0d+02*dmax1(0.0d+00,x(1)**2-x(2))
     4      +1.8d+02*dmax1(0.0d+00,x(3)**2-x(4))
           
        CASE(4)                                                         !   (DC1-4)
           f = ABS(x(1)-1.0D+00)+ABS(x(3)-1.0D+00)
     1      +4.95D+00*(ABS(x(2)+x(4)-2.0D+00))
     2      +1.01D+01*(ABS(x(2)-1.0D+00)+ABS(x(4)-1.0D+00))
     3      +2.0D+02*dmax1(0.0d+00,ABS(x(1))-x(2))
     4      +1.8D+02*dmax1(0.0d+00, ABS(x(3))-x(4))

        CASE(5)                                                         !   (DC1-5) 
           f = 9.0d+00-8.0d+00*x(1)-6.0d+00*x(2)-4.0d+00*x(3)+x(3)**2
      
!**********************************************************************
! GROUP 2 DC2-1 (nselect=i=6) to DC2-4 (nselect=i=9)
!**********************************************************************          
       CASE(6)                                                          !   (DC2-1)  
           f=x(1)**2+4.0d+00*x(2)**2+6.0d+00*x(3)**2    
           
       CASE(7)                                                          !   (DC2-2)
          f=4.0D+00*x(1)**2+2.0D+00*x(2)**2+2.0D+00*x(3)**2-3.3D+01*x(1)
     1     +1.6D+01*x(2)-2.4D+01*x(3)      
                    
       CASE(8)                                                          !   (DC2-3)   
           f=(x(1)-1.0d+01)**2+5.0d+00*(x(2)-1.2d+01)**2+x(3)**4
     1      +3.0d+00*(x(4)-1.1d+01)**2+1.0d+01*x(5)**2+7.0d+00*x(6)**2
     2      +x(7)**4-1.0d+01*x(6)-8.0d+00*x(7)  
                
       CASE(9)                                                          !   (DC2-4)
           f=0.0d+00
           DO i=1,m-1
            z = x(i)**4+x(i+1)**2
            v = (2.0d+00-x(i))**2+(2.0d+00-x(i+1))**2
            w = 2.0d+00*EXP(-x(i)+x(i+1)+1.0d+00)
            f = f+dmax1(z,v,w)
           END DO  
!**********************************************************************
! GROUP 3 DC3-1 (nselect=i=10) to DC3-12 (nselect=i=21)
!**********************************************************************        
       ...
       CASE(12)                                                          !   (DC3-3)
         f = x(1)**2+3.0d+00*x(2)**2+x(3)

       CASE(13)                                                          !   (DC3-4)
         f = 1.0d+02*(x(1)**2+x(2)**2)+(1-x(1))**2
       ...
       CASE(15)                                                          !   (DC3-6)
        f=0.0d+00
        DO i=1, m-1
         f = f+x(i)**2+(x(i+1)-1.0d+00)**2+x(i+1)-1.0d+00
        END DO
       
       CASE(16)                                                          !   (DC3-7)
        f=ABS(x(1)-1.0d+00)+2.0d+02*DMAX1(0.0d+00, ABS(x(1))-x(2))                  
        a1=x(1)**2+x(2)**2+ABS(x(2))
        a2=x(1)+x(1)**2+x(2)**2+ABS(x(2))-5.0d-01
        a3=ABS(x(1)-x(2))+ABS(x(2))-1.0d+00
        a4=x(1)+x(1)**2+x(2)**2
        f=f+1.0d+01*DMAX1(a1,a2,a3,a4)
       
       CASE(17)                                                          !   (DC3-8)
        f=x(1)**2+(x(1)-1.0d+00)**2+2.0d+00*(x(1)-2.0d+00)**2 
     1   +(x(1)-3.0d+00)**2+2.0d+00*x(2)**2+(x(2)-1.0d+00)**2 
     2   +2.0d+00*(x(2)-2.0d+00)**2+x(3)**2+(x(3)-1.0d+00)**2 
     3   +2.0d+00*(x(3)-2.0d+00)**2+(x(3)-3.0d+00)**2 
     4   +2.0d+00*x(4)**2+(x(4)-1.0d+00)**2+2.0d+00*(x(4)-2.0d+00)**2

       CASE(18)                                                          !   (DC3-9)
         f = x(2)+1.0d-01*(x(1)**2+x(2)**2)+1.0d+01*dmax1(-x(2),0.0d+00)

       CASE(19)                                                           !   (DC3-10)
         a1 = x(1)**4+x(2)**2
         a2 = (2.0d+00-x(1))**2+(2.0d+00-x(2))**2 
         a3 = 2.0d+00*EXP(-x(1)+x(2))
         a4 = x(1)**2-2.0d+00*x(1)+x(2)**2-4.0d+00*x(2)+4.0d+00
         a5 = 2.0d+00*x(1)**2-5.0d+00*x(1)+x(2)**2-2.0d+00*x(2)+4.0d+00 
         a6 = x(1)**2+2.0d+00*x(2)**2-4.0d+00*x(2)+1.0d+00               
         f = DMAX1(a1,a2,a3)+a4+a5+a6

       CASE(20)                                                          !   (DC3-11)
         f = ABS(x(1)-1.0d+00)+2.0d+02*DMAX1(0.0d+00,ABS(x(1))-x(2))
           
        CASE(21)                                                          !   (DC3-12) 
         f = ABS(x(1)-1.0d+00)+ABS(x(3)-1.0d+00)+4.95d+00*(ABS(x(2)+x(4)
     1    -2.0d+00))+1.01d+01*(ABS(x(2)-1.0d+00)+ABS(x(4)-1.0d+00))
     2    +2.0d+02*DMAX1(0.0d+00,ABS(x(1))-x(2))
     3    +1.8d+02*DMAX1(0.0d+00,ABS(x(3))-x(4))       
...
c--------------------------------------------------------------------------
      subroutine func2(x,f)
      implicit double precision (a-h,o-z)
      PARAMETER(maxvar=2000)
      double precision x(maxvar) 
      COMMON /cnf2/nf2,/cnselect/nselect, /csize/m
      nf2=nf2+1
...
!**********************************************************************          
      ...
      CASE(2)                                                            !   (DC1-2)
         f=1.0d+02*(ABS(x(1))-x(2))       
                 
      CASE(3)                                                            !   (DC1-3)    
         f=1.0d+02*(x(1)**2-x(2))
         f=f+9.0d+01*(x(3)**2-x(4))
         
      CASE(4)                                                            !   (DC1-4)
         f = 1.0D+02*(ABS(x(1))-x(2))+4.95D+00*ABS(x(2)-x(4))
     1       +9.0D+01*(ABS(x(3))-x(4))
...
!**********************************************************************         
       ...
       CASE(14)                                                          !   (DC3-5)
        ...
        CASE(15)                                                         !   (DC3-6)
          f=0.0d+00
          DO i=1,m-1
           f=f+ABS(x(i))+ABS(x(i+1)-1.0d+00)
          END DO
       
        CASE(16)                                                         !   (DC3-7)
          f=1.0d+02*(ABS(x(1))-x(2))+1.0d+01*(x(1)**2+x(2)**2+ABS(x(2)))
       
       CASE(17)                                                          !   (DC3-8)
          a=(x(1)-2.0d+00)**2+x(2)**2
          b=(x(3)-2.0d+00)**2+x(4)**2
          f=dmax1(a,b)
          a=(x(1)-2.0d+00)**2+(x(2)-1.0d+00)**2
          b=(x(3)-2.0d+00)**2+(x(4)-1.0d+00)**2
          f=f+dmax1(a,b)
          a=(x(1)-3.0d+00)**2+x(2)**2
          b=(x(3)-3.0d+00)**2+x(4)**2
          f=f+dmax1(a,b)
          a=x(1)**2+(x(2)-2.0d+00)**2
          b=x(3)**2+(x(4)-2.0d+00)**2
          f=f+dmax1(a,b)
          a=(x(1)-1.0d+00)**2+(x(2)-2.0d+00)**2
          b=(x(3)-1.0d+00)**2+(x(4)-2.0d+00)**2
          f=f+dmax1(a,b)
       ...
       CASE(19)                                                         !   (DC3-10)
          ...
          a5 = 2.0d+00*x(1)**2-5.0d+00*x(1)+x(2)**2-2.0d+00*x(2)+4.0d+00 
          a6 = x(1)**2+2.0d+00*x(2)**2-4.0d+00*x(2)+1.0d+00               
          f = DMAX1(a4+a5,a4+a6,a5+a6)
    
       CASE(20)                                                         !   (DC3-11)
          f = 1.0d+02*(ABS(x(1))-x(2))       
       
       CASE(21)                                                         !   (DC3-12)    
          f = 4.95d+00*(ABS(x(2)-x(4)))+9.0d+01*(ABS(x(3))-x(4))
     1     +1.0d+02*(ABS(x(1))-x(2)) 
       END SELECT
       return
       end
      
c--------------------------------------------------------------------------
! Gradients of DC componenets of objective f1 & f2
c--------------------------------------------------------------------------
...
!**********************************************************************          
      CASE(1)                                                           !   (DC1-1)
        ...
        grad(2) = 0.0d+00                
        z1=x(1)**2-x(2)
        IF(z1.ge.0.0d+00) THEN
          grad(1) = grad(1) + 4.0d+02*x(1)                   
          grad(2) = -2.0d+02
        END IF      
      ...
      CASE(3)                                                           !   (DC1-3)
        ...
        grad(1)= grad(1)+2.0d+00*(x(1)-1.0d+00)
        z1=x(1)**2-x(2)
        IF(z1.ge.0.0d+00) THEN
         grad(1)=grad(1)+4.0d+02*x(1)
        ...
        END IF
        z2=x(3)**2-x(4)
        IF(z2.ge.0.0d+00) THEN
         grad(3)=grad(3)+3.6d+02*x(3)
         grad(4)=grad(4)-1.8d+02
        END IF          
        grad(3)=grad(3)+2.0d+00*(x(3)-1.0d+00)           
        grad(2)=grad(2)+2.02d+01*(x(2)-1.0d+00)
        grad(4)=grad(4)+2.02d+01*(x(4)-1.0d+00)          
        grad(2)=grad(2)+9.9d+00*x(2)
        grad(4)=grad(4)+9.9d+00*x(4)          
      ...
      CASE(4)                                                           !   (DC1-4)
          ...
          z2=ABS(x(3))-x(4)
          IF(z2.ge.0.0D+00) THEN
           IF(x(3).le.0.0D+00) THEN 
                grad(3) = grad(3)-1.8D+02
              ELSE
                grad(3) = grad(3)+1.8D+02  
           END IF                    
           grad(4) = grad(4)-1.8D+02
          ...
          IF (x(2)<=1.0D+00) THEN
              grad(2) = grad(2)-1.01D+01  
            ELSE
              grad(2) = grad(2)+1.01D+01
          END IF         
                    
          IF (x(4)<=1.0D+00) THEN
              grad(4) = grad(4)-1.01D+01  
            ELSE
              grad(4) = grad(4)+1.01D+01
          END IF 
          z3=x(2)+x(4)-2.0D+00          
...
!**********************************************************************          
        ...
        CASE(7)                                                           !   (DC2-2)
          grad(1) = 8.0D+00*x(1)-3.3D+01
          grad(2) = 4.0D+00*x(2)+1.6D+01
          grad(3) = 4.0D+00*x(3)-2.4D+01
          
        CASE(8)                                                           !   (DC2-3)   
           grad(1)=2.0d+00*x(1)-2.0d+01
           grad(2)=1.0d+01*x(2)-1.2d+02
           grad(3)=4.0d+00*x(3)**3
           grad(4)=6.0d+00*x(4)-6.6d+01
           grad(5)=2.0d+01*x(5)
           grad(6)=1.4d+01*x(6)-1.0d+01
           grad(7)=4.0d+00*x(7)**3-8.0d+00
        ...
        CASE(9)                                                         !   (DC2-4)
          ...
          DO i=1, m-1
           ...
           END IF 
           IF (p==v) THEN
            grad(i) = grad(i)+2.0d+00*x(i)-4.0d+00
            grad(i+1) = grad(i+1)+2.0d+00*x(i+1)-4.0d+00
           END IF
           IF(p==w) THEN
            grad(i) = grad(i)-w
            grad(i+1) = grad(i+1)+w
...
!**********************************************************************         
       ...
       CASE(13)                                                           !   (DC3-4)
          grad(1) = 2.0d+02*x(1)-2.0d+00*(1.0d+00-x(1))
          grad(2) = 2.0d+02*x(2)   
       ...
       CASE(16)
          ...
          a(2) = x(1)+x(1)**2+x(2)**2+ABS(x(2))-5.0d-01
          a(3) = ABS(x(1)-x(2))+ABS(x(2))-1.0d+00
          a(4) = x(1)+x(1)**2+x(2)**2             
          ind = 1
          a5=a(1)
          DO i = 2,4
          ...
          END DO
          IF (ind==1) THEN
            grad(1) = grad(1)+2.0d+01*x(1)
            grad(2) = grad(2)+2.0d+01*x(2)
            IF (x(2)<=0.0d+00) THEN
                grad(2) = grad(2)-1.0d+01
              ELSE
                grad(2) = grad(2)+1.0d+01
            END IF
          END IF
          IF (ind==2) THEN
             grad(1) = grad(1)+1.0d+01+2.0d+01*x(1)
             grad(2) = grad(2)+2.0d+01*x(2)
             IF (x(2)<=0.0d+00) THEN
                grad(2) = grad(2)-1.0d+01
              ELSE
       ...
       CASE(17)                                                         !   (DC3-8)
         grad(1)=1.0d+01*x(1)-1.6d+01
         grad(2)=1.0d+01*x(2)-1.0d+01 
         grad(3)=1.0d+01*x(3)-1.6d+01
         grad(4)=1.0d+01*x(4)-1.0d+01  

       CASE(18)                                                           !   (DC3-9)
          grad(1) = 2.0d-01*x(1)
          grad(2) = 1.0d+00+2.0d-01*x(2) 
          IF (x(2)<=0.0d+00) THEN 
             grad(2) = grad(2)-1.0d+01
          END IF  
       ...
       CASE(19)                                                         !   (DC3-10)
          ...
          f=dmax1(a1,a2,a3)
          IF(a1.eq.f) THEN
            grad(1) = 4.0d+00*x(1)**3 
            grad(2) = 2.0d+00*x(2)                
          END IF
          IF(a2.eq.f) THEN
            grad(1) = 2.0d+00*x(1)-4.0d+00
            grad(2) = 2.0d+00*x(2)-4.0d+00                            
          END IF
          IF(a3.eq.f) THEN
            grad(1) = -a3
            grad(2) = a3                 
          END IF
          grad(1) = grad(1)+8.0d+00*x(1)-7.0d+00
          grad(2) = grad(2)+8.0d+00*x(2)-1.0d+01
...
c--------------------------------------------------------------------------
      ...
      COMMON /csize/m,/cngrad2/ngrad2,/cnselect/nselect, /cn/n
       ngrad2=ngrad2+1
...
!**********************************************************************          
       ...
       CASE(16)                                                           !   (DC3-7) 
         grad(1) = 2.0d+01*x(1)
         grad(2) = -1.0d+02+2.0d+01*x(2)
         IF (x(1)<=0.0d+00) THEN 
              grad(1) = grad(1)-1.0d+02
            ELSE
              grad(1) = grad(1)+1.0d+02
         END IF
         IF (x(2)<=0.0d+00) THEN 
       ...
       CASE(17)                                                           !   (DC3-8) 
         ...
         b=(x(3)-2.0d+00)**2+x(4)**2
         IF (a>=b) THEN 
             grad(1)=grad(1)+2.0d+00*(x(1)-2.0d+00)
             grad(2)=grad(2)+2.0d+00*x(2)
           ELSE 
             grad(3)=grad(3)+2.0d+00*(x(3)-2.0d+00)
             grad(4)=grad(4)+2.0d+00*x(4)
         END IF
         a=(x(1)-2.0d+00)**2+(x(2)-1.0d+00)**2
         b=(x(3)-2.0d+00)**2+(x(4)-1.0d+00)**2
         IF (a>=b) THEN 
             grad(1)=grad(1)+2.0d+00*(x(1)-2.0d+00)
             grad(2)=grad(2)+2.0d+00*(x(2)-1.0d+00)
            ELSE 
             grad(3)=grad(3)+2.0d+00*(x(3)-2.0d+00)
             grad(4)=grad(4)+2.0d+00*(x(4)-1.0d+00)
         END IF
         a=(x(1)-3.0d+00)**2+x(2)**2
         b=(x(3)-3.0d+00)**2+x(4)**2
         IF (a>=b) THEN 
             grad(1)=grad(1)+2.0d+00*(x(1)-3.0d+00)
             grad(2)=grad(2)+2.0d+00*x(2)
            ELSE 
             grad(3)=grad(3)+2.0d+00*(x(3)-3.0d+00)
             grad(4)=grad(4)+2.0d+00*x(4) 
         END IF
         a=x(1)**2+(x(2)-2.0d+00)**2
         b=x(3)**2+(x(4)-2.0d+00)**2
         IF (a>=b) THEN 
             grad(1)=grad(1)+2.0d+00*x(1)
             grad(2)=grad(2)+2.0d+00*(x(2)-2.0d+00)
            ELSE 
             grad(3)=grad(3)+2.0d+00*x(3)
             grad(4)=grad(4)+2.0d+00*(x(4)-2.0d+00)
         END IF
         a=(x(1)-1.0d+00)**2+(x(2)-2.0d+00)**2
         b=(x(3)-1.0d+00)**2+(x(4)-2.0d+00)**2
         IF (a>=b) THEN 
       ...
       CASE(19)                                                           !   (DC3-10) 
         ...
         f=dmax1(a7,a8,a9)
         IF(a7.eq.f) THEN
           grad(1)=6.0d+00*x(1)-7.0d+00
           grad(2)=4.0d+00*x(2)-6.0d+00
         END IF
         IF(a8.eq.f) THEN
           grad(1) = 4.0d+00*x(1)-2.0d+00
           grad(2) = 6.0d+00*x(2)-8.0d+00
         END IF
         IF(a9.eq.f) THEN
           grad(1) = 6.0d+00*x(1)-5.0d+00
           grad(2) = 6.0d+00*x(2)-6.0d+00
         END IF               
...
!**********************************************************************          
      CASE(1)                                                           !   (DC1-1)
        ! first constraint
          g1(1) = 3.0d+00*x(1)+2.0d+00*x(2)**2-3.0d+00
          g2(1) = 0.0d+00
        !second constraint  
          g1(2) = x(1)**2-6.0d+00*x(2)+1.0d+00       
          g2(2) = 0.0d+00
      ...
      CASE(2)                                                           !   (DC1-2)
        ! first constraint
          ...
          g2(1) = 0.0d+00            
        ! second constraint
          g1(2) = x(1)**2-3.0d+00*x(2)-2.0d+00       
          g2(2) = 0.0d+00       
          
      CASE(3)                                                           !   (DC1-3)    
        ! first constraint
         g1(1)=2.0d+00*x(1)**2+x(2)**2-2.0d+00*x(3)+1.0d+01*x(4)-2.0d+00
          g2(1)=0.0d+00            
        ! second constraint
          g1(2)=x(1)-2.0d+00*x(2)+x(3)**2+x(4)**2-5.0d+00       
          g2(2)=0.0d+00
        ! third constraint
          g1(3)=2.0d+00*x(1)**2+x(2)**2+x(3)**2+2.0d+00*x(4)**2-2.5d+01
          g2(3)=0.0d+00
           
      CASE(4)                                                           !   (DC1-4)
        ! first constraint
         g1(1)=2.0D+00*x(1)**2+x(2)**2-2.0D+00*x(3)+3.0D+00*x(4)-2.0D+00
          g2(1)=0.0d+00  
      ...
      CASE(5)                                                           !   (DC1-5)
        ...
        ! third constraint
          ...
          g2(3)=0.0d+00
        ! forth constraint
          g1(4)=x(1)+x(2)+2.0d+00*x(3)-1.3d+01      
          g2(4)=0.0d+00             
!**********************************************************************
! GROUP 2 DC2-1 (nselect=i=6) to DC2-4 (nselect=i=9)
!**********************************************************************          
      CASE(6)                                                           !   (DC2-1)
        ! first constraint
          g1(1)=x(1)**2+x(3)**2-1.0d+00
          g2(1)=x(2)**2-x(1)+1.0d+00
           
        !second constraint  
          g1(2)=x(2)**2+x(3)**2-1.0d+01       
          g2(2)=3.0d+00*x(1)**2-1.1d+01
           
        !third constraint  
          g1(3)=x(1)**2+x(2)**2-1.0d+00     
          g2(3)=EXP(2.0d+00*x(3)-1.0d+00)+1.0d+00
                  
      CASE(7)                                                           !   (DC2-2)
        ! first constraint
          g1(1)=2.0D+00*x(1)**2+3.0D+00*x(1)-1.0D+00
          g2(1)=2.0D+00*x(2)**2+7.0D+00
           
        !second constraint  
          g1(2)=3.0D+00*x(1)**2+4.0D+00*x(1)-1.0D+01       
          g2(2)=x(3)**2+1.0D+00
               
      CASE(8)                                                           !   (DC2-3)
        ! first constraint
           g1(1)=2.0d+00*x(1)**2+3.0d+00*x(2)**4+x(3)-1.0d+01
           g2(1)=4.0d+00*x(4)**2+5.0d+00*x(5)+7.0d+00       
                       
        ! second constraint
           g1(2)=7.0d+00*x(1)+3.0d+00*x(2)+1.0d+01*x(3)**2+x(4)
     1       -x(5)-1.0d+02
           g2(2)=0.0d+00
           
        ! third constraint
           g1(3)=x(2)**2+8.0d+00*x(7)-4.0d+01      
           g2(3)=2.3d+01*x(1)+6.0d+00*x(6)**2-2.0d+01
           
        ! forth constraint
           g1(4)=4.0d+00*x(1)**2+3.0d+00*x(4)**2-5.0d+00*x(6)-5.0d+00
           g2(4)=4.0d+00*x(2)**2+2.0d+00*x(3)**2+1.4d+01*x(7)+2.0d+01 
      ...
      CASE(9)                                                           !   (DC2-4)
        ! first constraint
          ...
          DO i=1,m-1
            g1(1)=g1(1)+x(i)**2
          END DO
          g2(1)=x(m)**2+4.0d+00*dble(m)   
                                 
        ! second constraint
          g1(2)=-1.0d+00
          DO i=1,m
            g1(2)=g1(2)+(x(i)-2.0d+00)**2                                
          END DO
          g2(2)=0.0d+00     
!**********************************************************************
! GROUP 3 DC3-1 (nselect=i=10) to DC3-12 (nselect=i=21)
!**********************************************************************        
       ...
       CASE(14)                                                           !   (DC3-5)
         ! first constraint
           g1(1)=-1.0d+01  
           DO i=1, m
            g1(1)=g1(1)+dble(i)*x(i)**2                             
           END DO
           g2(1)=0.0d+00        
           
         ! second constraint
           g1(2)=4.0d+00*x(1)+5.0d+00*x(2)-1.0d+01*x(7)-6.0d+00*x(9)-2.0      
           g2(2)=0.0d+00

         ! third constraint
           g1(3)=0.0d+00 
           g2(3)=3.0*ABS(x(3)-2.0)-2.0d+00*x(4)+6.0d+00*ABS(x(6))-2.0  
               
         ! forth constraint
           g1(4)=5.0d+00*x(5)**2+4.0d+00*ABS(x(8)-1.0d+00)-6.0d+00       
           g2(4)=3.0d+00*ABS(x(10)-3.0d+00)+4.0d+00   
       ...
       CASE(17)                                                           !   (DC3-8)
         ! first constraint
           g1(1)=0.0d+00
           a=x(1)+x(2)+x(3)+x(4)-5.50d+00
           b=x(1)**2+x(2)**2+x(3)**2+x(4)**2-1.0d+01
           g2(1)=dmax1(a,b)
...
!**********************************************************************          
      ...
      CASE(3)                                                           !   (DC1-3)      
        ...
        ! third constraint  
          grad1(3,1) = 4.0d+00*x(1)
          grad1(3,2) = 2.0d+00*x(2)
          grad1(3,3) = 2.0d+00*x(3)
          grad1(3,4) = 4.0d+00*x(4)   
          grad2(3,1) = 0.0d+00
          grad2(3,2) = 0.0d+00 
          grad2(3,3) = 0.0d+00  
          grad2(3,4) = 0.0d+00     
...
!**********************************************************************          
      CASE(6)                                                           !   (DC2-1) 
         ! first constraint
           grad1(1,1)=2.0d+00*x(1) 
           grad1(1,2)=0.0d+00 
           grad1(1,3)=2.0d+00*x(3)
         ...
         ! third constraint  
           ...
           grad2(3,1)=0.0d+00 
           grad2(3,2)=0.0d+00
           grad2(3,3)=2.0d+00*EXP(2.0d+00*x(3)-1.0d+00)            
               
      CASE(7)                                                           !   (DC2-2) 
         ! first constraint
           grad1(1,1)=4.0D+00*x(1)+3.0D+00 
           grad1(1,2)=0.0d+00 
           grad1(1,3)=0.0D+00
              
           grad2(1,1)=0.0D+00 
           grad2(1,2)=4.0D+00*x(2)
           grad2(1,3)=0.0D+00
           
         ! second constraint  
           grad1(2,1)=6.0D+00*x(1)+4.0D+00
           grad1(2,2)=0.0D+00
           grad1(2,3)=0.0D+00     
              
           grad2(2,1)=0.0D+00
           grad2(2,2)=0.0D+00
           grad2(2,3)=2.0D+00*x(3)
                      
      CASE(8)                                                           !   (DC2-3)  
         ! first constraint
           grad1(1,1) =4.0d+00*x(1) 
           grad1(1,2) =1.2d+01*x(2)**3 
           grad1(1,3) =1.0d+00 
           grad1(1,4) =0.0d+00 
           grad1(1,5) =0.0d+00 
           grad1(1,6) =0.0d+00 
         ...
         ! third constraint  
           ...
           grad2(3,4) = 0.0d+00
           grad2(3,5) = 0.0d+00 
           grad2(3,6) = 1.2D+00*x(6) 
           grad2(3,7) = 0.0d+00        
           
         ! forth constraint  
           grad1(4,1) =8.0D+00*x(1) 
           grad1(4,2) =0.0d+00 
           grad1(4,3) =0.0d+00 
           grad1(4,4) =6.0D+00*x(4) 
           grad1(4,5) =0.0d+00 
           grad1(4,6) =-5.0D+00 
           grad1(4,7) =0.0d+00    
             
           grad2(4,1) = 0.0d+00
           grad2(4,2) = 8.0D+00*x(2) 
           grad2(4,3) = 4.0D+00*x(3) 
           grad2(4,4) = 0.0d+00        
           grad2(4,5) = 0.0d+00 
           grad2(4,6) = 0.0d+00  
           grad2(4,7) = 1.4D+01
      ...
      CASE(9)                                                           !   (DC2-4) 
         ! first constraint
           ...
           END DO
           grad1(1,m) = 0.0d+00
           grad2(1,m) = 2.0d+00*x(m)               
           
         ! second constraint  
           DO i=1,m
            grad1(2,i) = 2.0d+00*(x(i)-2.0d+00)
            grad2(2,i) = 0.0d+00
           END DO
!**********************************************************************
! GROUP 3 DC3-1 (nselect=i=10) to DC3-12 (nselect=i=21)
      ...
      CASE(10)                                                           !   (DC3-1) 
       ...
       CASE(14)                                                         !   (DC3-5) 
         ...
         CASE(15)                                                           !   (DC3-6) 
           ...
           END DO                         
           DO i = 1,m1
            grad1(1,2*i)=2.0d+00*x(2*i)
           END DO
           m1 = ceiling(m/2.0d+00)                           
           DO i = 1,m1
            grad2(1,2*i-1)=2.0d+00*x(2*i-1)
           END DO   
...
c     &                  MP1    = M+1,
     ...
     &                  X(N),F(M+MG),GAM(MP1),BL(N),BU(N),XX(N),
