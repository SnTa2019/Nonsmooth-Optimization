...
PROGRAM bem
    ...
    TYPE(bundle_element), allocatable, DIMENSION(:) :: bundle !
    TYPE(tbundle_element), allocatable, DIMENSION(:) :: tbundle !
    ...
    REAL(KIND=prec) :: &
        ...
        bline, &                                                    ! Line search parameter: it can be between 1 and 50.
    ...
    INTEGER :: &
        ...
        nfe,nge1,nge2,&                                       ! Number of function and subgradient evaluations. 
    ...
    allocate(bundle(size_b),tbundle(size_tb)) !
    ...
    ! Initialization
    nres=0
    atol = 0.000001_prec                                   ! Tolerance for linearization error.
    ...
    WRITE(42,*)
    WRITE(42,*) 'Problem formulation: f1 = ',pnf1 ,'f2 =',pnf2 ,'x1 = ',sp,'n =',n 
    WRITE(42,*)
   ...
   outerloop: DO                                               !WHILE (nit <= mit) 
       ...
       innerloop: DO
           ...
           IF (v >= -tol) THEN
               ...
               WRITE(41,*) ' Nf = ',nfe,' Ng1 = ',nge1,' Ng2 = ',nge2,' f = ',f_current,&
                   &' CPU time = ',finish_time-start_time,'  nff=',nff,'  nffl=',nffl 
               WRITE(41,*)  
               WRITE(41,*) '  ----  x_optimal ----- '    
                
               PRINT*,'STOP from Step 2',-v,dnorm
           ...
           !***********************************************************************  
           IF(DOT_PRODUCT(d_old-d,d_old-d) /= 0.0_prec) THEN 
               ...
               nfe = nfe + 1   
               IF (f1-f2-f_current < sdp*v) THEN
                   PRINT*,'    Step 5: Descent direction 2'     
                   EXIT innerloop                             ! Descent direction.
               END IF
                   ...
                   IF(tt*dnorm > teta) THEN
                       ...
                       IF (fd < zero) THEN    
                           ...
                           DO WHILE (tt*dnorm > teta)       
                               ...
                               nff = nff + 1                    
                               IF (f1-f2-f_current < tt*sdp*v) THEN
                                   PRINT*,'Exit innerloop from Step8, niit=',niit,tt
                                   EXIT innerloop             ! Descent direction.
                               END IF
                           END DO
                       END IF
       ...
       IF (ls == 1) THEN                                     ! Simple line search is used
           ...
           linesearch: DO
               ...
               nfe = nfe + 1
               nffl = nffl + 1
               IF (fy-f_current <= tt1*sdp1*v) THEN 
                   x = y
                   f1_c = f1
                   f2_c = f2
                   f_current1 = fy
